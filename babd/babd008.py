...
while len(sentences) - 1 > current_sentence:
    ...
    train_model(model, criterion, optimizer, input_tensor, output_tensor, input_attention_mask, output_attention_mask)
    
    if halt_training:
        command = input('Enter "resume" to continue training: ')
        if command == "resume":
            halt_training = False
            train_model(model, criterion, optimizer, input_tensor, output_tensor, input_attention_mask, output_attention_mask)
