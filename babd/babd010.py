...
def train_model(model, criterion, optimizer, input_tensor, output_tensor, input_attention_mask, output_attention_mask, epoch_num):
    global halt_training
    epoch = 0
    loss = 100
...
def load_model(model, model_path):
    if os.path.exists(model_path):
        ...
        print('No pre-existing model found. Starting fresh.')
...
while len(sentences) - 1 > current_sentence:
    ...
    final_output, epoch_num = train_model(model, criterion, optimizer, input_tensor, output_tensor, input_attention_mask, output_attention_mask, epoch_num)
