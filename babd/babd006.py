...
# Start create one common function for this
...
output_array, output_attention_mask_array, output_position_array = processor.process_sentence(output_text)

# Convert arrays to tensors
input_tensor = torch.tensor(input_array)
output_tensor = torch.tensor(output_array)
input_attention_mask = torch.tensor(attention_mask_array,dtype=torch.float)
output_attention_mask = torch.tensor(output_attention_mask_array,dtype=torch.float)

# end
# input_pos_tensor = torch.tensor(position_array,dtype=torch.float).unsqueeze(1)
# output_pos_tensor = torch.tensor(output_position_array,dtype=torch.float).unsqueeze(1)
